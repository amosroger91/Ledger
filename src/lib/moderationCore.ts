// ============================================================
//  moderationCore — the PURE, dependency-free moderation evaluation.
//  This is the single source of truth: moderationService (main thread)
//  and feedRank (in the feed worker) both call evaluateModeration, so
//  the composer and the ranked feed never disagree. The viewer's
//  web-of-trust values (block/mute/score/vouches) are passed IN —
//  computed from trustService on main, lib/trustMath in the worker —
//  so this module imports nothing and runs anywhere.
// ============================================================
...
/** Viewer web-of-trust toward this author, precomputed by the caller. */
export interface TrustInput { blocked: boolean; muted: boolean; score: number; vouchCount: number; }

/** The layered evaluation. Returns a graded, explainable verdict. */
export function evaluateModeration(text: string, ctx: EvalContext, tw: TrustInput): ModerationVerdict {
  ...
  if (ctx.authorPk && (tw.blocked || tw.muted)) {
    const blocked = tw.blocked;
  ...
  const trust = ctx.authorPk ? tw.score : 0;
  ...
  // ----- web of trust -----
  if (trust >= 0.5) signals.push({ label: "vouched by your circle", weight: -Math.min(0.8, trust * 0.5), detail: `${tw.vouchCount} vouches` });
  ...
  const reasoning = reason(action, signals, ctx);
...
function reason(action: ModerationAction, signals: ModerationSignal[], ctx: EvalContext): string {
