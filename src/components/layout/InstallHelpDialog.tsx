...
export default function InstallHelpDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function install() {
    ...
    if (outcome === "accepted") { toast("Installing Ledgr…", "success"); onClose(); }
  }

  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { backgroundImage: "none" } }}>
      <DialogTitle sx={{ pb: 0.5 }}>Install Ledgr on your phone</DialogTitle>
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Ledgr installs straight from the browser — no app store. It then opens full-screen like a native app and works offline.
        </Typography>
        ...
        <Typography variant="caption" color="text.secondary" sx={{ display: "block", textAlign: "center" }}>
          On a computer? Scan this with your phone's camera to open Ledgr there, then follow the steps below.
        </Typography>
