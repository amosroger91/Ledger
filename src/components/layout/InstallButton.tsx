...
export default function InstallButton({ compact }: { compact: boolean }) {
  ...
  async function onClick() {
    if (canPrompt) {
      ...
      if (outcome === "accepted") toast("Installing Ledgr…", "success");
      return;
    }
  ...
  return (
    <>
      ...
      <Dialog open={iosOpen} onClose={() => setIosOpen(false)} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontWeight: 800 }}>Install Ledgr on your iPhone</DialogTitle>
        <DialogContent>
          ...
          <Typography sx={{ mt: 2 }} variant="caption" color="text.secondary">
            Ledgr then opens full-screen, like a native app. Make sure you're in Safari — other iOS browsers can't add to the home screen.
          </Typography>
        </DialogContent>
        <DialogActions>
